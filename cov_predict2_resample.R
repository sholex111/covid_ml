...
raw_data_stripped <-  raw_data %>% select ( id,los, severity, map, age_27, creatinine, procalcitonin,
...
# Separate the data into the death and survival groups
death_cases <- raw_data_stripped %>% filter(death == 1)
survival_cases <- raw_data_stripped %>% filter(death == 0)



# Randomly undersample the survival cases to match the number of death cases
set.seed(1234)  #for reproducibility
balanced_survival_cases <- survival_cases %>% sample_n(nrow(death_cases))


# Combine both classes back into one balanced dataset

balanced_data <- bind_rows(death_cases, balanced_survival_cases)
